...
static int keyio_init(void)
{
	...
	/* 提取GPIO */
    keyinputdev.irqkeydesc[0].gpio = of_get_named_gpio(keyinputdev.nd ,"key-gpio", 0);
    if (keyinputdev.irqkeydesc[0].gpio < 0) {
        printk("can't get key%d\r\n", 0);
    }
	
	/* 初始化key所使用的IO，并且设置成中断模式 */

    memset(keyinputdev.irqkeydesc[0].name, 0, sizeof(name));	/* 缓冲区清零 */
    sprintf(keyinputdev.irqkeydesc[0].name, "KEY%d", 0);		/* 组合名字 */
    gpio_request(keyinputdev.irqkeydesc[0].gpio, name);
    gpio_direction_input(keyinputdev.irqkeydesc[0].gpio);	
    keyinputdev.irqkeydesc[0].irqnum = irq_of_parse_and_map(keyinputdev.nd, 0);
    ...
    ret = request_irq(keyinputdev.irqkeydesc[0].irqnum, keyinputdev.irqkeydesc[0].handler, 
                        IRQF_TRIGGER_FALLING|IRQF_TRIGGER_RISING, keyinputdev.irqkeydesc[0].name, &keyinputdev);
    if(ret < 0){
...
static void __exit keyinput_exit(void)
{
	...
	/* 释放中断 */
    free_irq(keyinputdev.irqkeydesc[0].irqnum, &keyinputdev);
