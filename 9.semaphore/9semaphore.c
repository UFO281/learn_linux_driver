...
static int devopen(struct inode *inode, struct file *filp)
{
    ...
    /*获取信号量 信号量-1，进入休眠状态的进程可以被信号打断*/
    /*如果信号量为0时候，在获取信号量会导致线程休眠，
    然后等上一个线程释放信号量之后，在执行*/
    if ( down_interruptible(&led.sema0) )
    {
        return -ERESTARTSYS;
    }

#if 0
    down(&led.sema0);   /*获取信号量，不能被信号打断*/
#endif
...
static int __init wlsdevinit(void)
{
    ...
    sema_init(&led.sema0,1);/*初始化信号量并且值为1*/
